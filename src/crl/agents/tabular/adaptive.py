...
from crl.predictors.tabular import Predictor
...
class AdaptiveAgentParams:
    ...
    discount: float = 1.0
    rng: int | None = None
    adaptation_method: Literal["absolute", "difference"] = "absolute"
...
class AdaptiveLearningAgent:
    def __init__(
        ...
        # either track the absolute mean, or shift in mean nonconformity score
        self.adaptation_method = params.adaptation_method
        if self.adaptation_method == "difference":
            self.last_mean_scores = deque([0.0], maxlen=1000)
    ...
    def observe(self, obs: Observation) -> None:
        ...
        # model learning - exponential moving average to adapt to distribution shifts
        self.N[s, a, s_prime] += 1  # keep counts for planning/analysis
        one_hot = np.zeros_like(self.world_model[s, a, :])
        one_hot[s_prime] = 1.0
        # Update world‑model probabilities using the current (possibly adaptive) learning rate
        self.world_model[s, a, :] = (1.0 - self.learning_rate) * self.world_model[
            s, a, :
        ] + self.learning_rate * one_hot

        # Online adaptive update - expected reward
        self.reward_model[s, a] = (1.0 - self.learning_rate) * self.reward_model[
            s, a
        ...
        # Previous approach
        if self.adaptation_method == "absolute":
            self.adaptation_factor = (
                1.0 + self.adaptation_scale * self.predictor.mean_score(s, a)
            )
        elif self.adaptation_method == "difference":
            current_mean = self.predictor.mean_score(s, a)
            # compare to the mean 100 steps ago
            delta_mean = abs(current_mean - self.last_mean_scores[0])
            self.adaptation_factor = 1.0 + self.adaptation_scale * delta_mean
            self.last_mean_scores.append(current_mean)
        else:
            raise ValueError(f"Unknown adaptation method: {self.adaptation_method}")
        # Update the learning rate
        self.learning_rate = np.clip(
            self.adaptation_factor * self.init_learning_rate,
            a_min=0.0,
            a_max=1.0,
        )
